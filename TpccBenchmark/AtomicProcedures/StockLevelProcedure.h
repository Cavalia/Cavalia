			...
			namespace AtomicProcedures{
				...
				public:
					...
					virtual bool Execute(EventTuple *param, CharArray &ret, const ExeContext &exe_context){
						...
						DB_QUERY(SelectKeyRecord(&context_, DISTRICT_TABLE_ID, std::string(d_key, sizeof(int)* 2), district_record, READ_ONLY, 0));
						...
						for (int o_id = d_next_o_id - 5; o_id < d_next_o_id; ++o_id){
							...
							DB_QUERY(SelectRecords(&context_, ORDER_LINE_TABLE_ID, 0, std::string(ol_key, sizeof(int)* 3), order_line_records, READ_ONLY, access_id));
