#pragma once
#ifndef __CAVALIA_TPCC_BENCHMARK_ATOMIC_PROCEDURES_ORDER_STATUS_SHARD_PROCEDURE_H__
#define __CAVALIA_TPCC_BENCHMARK_ATOMIC_PROCEDURES_ORDER_STATUS_SHARD_PROCEDURE_H__
	...
	namespace Benchmark{
		namespace Tpcc{
			namespace ShardProcedures{
				class OrderStatusShardProcedure : public StoredProcedure{
				public:
					OrderStatusShardProcedure(const size_t &txn_type) : StoredProcedure(txn_type){
						context_.is_read_only_ = true;
						order_line_records = new SchemaRecords(15);
					}
					virtual ~OrderStatusShardProcedure(){}
					...
					virtual bool Execute(TxnParam *param, CharArray &ret, const ExeContext &exe_context){
						...
						int partition_id = (order_status_param->w_id_ - 1) % partition_count_;
						...
						DB_QUERY(SelectRecord(&context_, ORDER_TABLE_ID, partition_id, 0, std::string(o_key, sizeof(int)* 3), order_record, READ_ONLY));
				...
				private:
					OrderStatusShardProcedure(const OrderStatusShardProcedure&);
					OrderStatusShardProcedure& operator=(const OrderStatusShardProcedure&);
