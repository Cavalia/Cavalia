			...
			namespace AtomicProcedures{
				...
				public:
					...
					virtual bool Execute(TxnParam *param, CharArray &ret, const ExeContext &exe_context){
						...
						DB_QUERY(SelectKeyRecord(&context_, ACCOUNTS_TABLE_ID, std::string((char*)(&balance_param->custid_), sizeof(int64_t)), custid_record, READ_ONLY, 0));
						assert(custid_record != NULL);
						SchemaRecord *savings_record = NULL;
						DB_QUERY(SelectKeyRecord(&context_, SAVINGS_TABLE_ID, std::string((char*)(&balance_param->custid_), sizeof(int64_t)), savings_record, READ_ONLY, 0));
						assert(savings_record != NULL);
						SchemaRecord *checking_record = NULL;
						DB_QUERY(SelectKeyRecord(&context_, CHECKING_TABLE_ID, std::string((char*)(&balance_param->custid_), sizeof(int64_t)), checking_record, READ_ONLY, 0));
