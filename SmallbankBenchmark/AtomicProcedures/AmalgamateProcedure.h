			...
			namespace AtomicProcedures{
				...
				public:
					...
					virtual bool Execute(TxnParam *param, CharArray &ret, const ExeContext &exe_context){
						...
						DB_QUERY(SelectKeyRecord(&context_, ACCOUNTS_TABLE_ID, std::string((char*)(&amalgamate_param->custid_0_), sizeof(int64_t)), custid_0_record, READ_ONLY, 0));
						SchemaRecord *custid_1_record = NULL;
						DB_QUERY(SelectKeyRecord(&context_, ACCOUNTS_TABLE_ID, std::string((char*)(&amalgamate_param->custid_1_), sizeof(int64_t)), custid_1_record, READ_ONLY, 1));
						assert(custid_0_record != NULL);
						assert(custid_1_record != NULL);
						SchemaRecord *custid_0_savings_record = NULL;
						DB_QUERY(SelectKeyRecord(&context_, SAVINGS_TABLE_ID, std::string((char*)(&amalgamate_param->custid_0_), sizeof(int64_t)), custid_0_savings_record, READ_WRITE, 0));
						SchemaRecord *custid_0_checking_record = NULL;
						DB_QUERY(SelectKeyRecord(&context_, CHECKING_TABLE_ID, std::string((char*)(&amalgamate_param->custid_0_), sizeof(int64_t)), custid_0_checking_record, READ_WRITE, 0));
						...
						DB_QUERY(SelectKeyRecord(&context_, CHECKING_TABLE_ID, std::string((char*)(&amalgamate_param->custid_1_), sizeof(int64_t)), custid_1_checking_record, READ_WRITE, 1));
