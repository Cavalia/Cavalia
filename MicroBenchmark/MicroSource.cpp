				...
				for (size_t i = 0; i < num_transactions_; ++i){
					...
					if (x <= workload_weights[0]){
						...
						if (source_type_ == RANDOM_SOURCE){
							...
							while (1){
								res1 = MicroRandomGenerator::GenerateInteger(1, num_accounts_);
								if ((res1 - 1) % partition_count_ == partition_id){
									break;
								}
							}
							bool remote = (MicroRandomGenerator::GenerateInteger(1, 100) <= (int)dist_ratio_);
							while (1){
								res2 = MicroRandomGenerator::GenerateInteger(1, num_accounts_);
								if (remote == true){
						...
						if (source_type_ == RANDOM_SOURCE){
							...
							while (1){
								res = MicroRandomGenerator::GenerateInteger(1, num_accounts_);
