...
namespace Cavalia{
	namespace Benchmark{
		namespace Micro{
			...
			void MicroPopulator::InsertAccountsRecord(const AccountsRecord* record_ptr){
				char *data = new char[MicroSchema::GenerateAccountsSchema()->GetSchemaSize()];
				SchemaRecord *accounts_record = new SchemaRecord(MicroSchema::GenerateAccountsSchema(), data);
			...
			void MicroPopulator::InsertSavingsRecord(const SavingsRecord* record_ptr){
				char *data = new char[MicroSchema::GenerateSavingsSchema()->GetSchemaSize()];
				SchemaRecord *savings_record = new SchemaRecord(MicroSchema::GenerateSavingsSchema(), data);
			...
			void MicroPopulator::InsertCheckingRecord(const CheckingRecord* record_ptr){
				char *data = new char[MicroSchema::GenerateCheckingSchema()->GetSchemaSize()];
				SchemaRecord *checking_record = new SchemaRecord(MicroSchema::GenerateCheckingSchema(), data);
			...
			AccountsRecord* MicroPopulator::GenerateAccountsRecord(const int& custid) const{
				AccountsRecord* record = new AccountsRecord();
				record->custid_ = static_cast<int64_t>(custid);
				std::string accountName = MicroRandomGenerator::GenerateAccountName(custid);
				assert(accountName.length() == ACCOUNT_NAME_LEN);
				memcpy(record->name_, accountName.c_str(), accountName.size());
				return record;
			}
			SavingsRecord* MicroPopulator::GenerateSavingsRecord(const int& custid) const{
				SavingsRecord* record = new SavingsRecord();
				record->custid_ = static_cast<int64_t>(custid);
				//int balance = RandomGenerator::GenerateGaussianInteger(MIN_BALANCE, MAX_BALANCE);
				int balance = MicroRandomGenerator::GenerateInteger(MIN_BALANCE, MAX_BALANCE);
				record->bal_ = static_cast<float>(balance);
				return record;
			}
			CheckingRecord* MicroPopulator::GenerateCheckingRecord(const int& custid) const{
